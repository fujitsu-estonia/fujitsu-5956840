...
public class DataInit implements CommandLineRunner {
    ...
    private void createBooking(Room room, LocalDate startDate, LocalDate endDate) {
        bookingRepository.save(Booking.builder().startDate(bookingDatesConverter.mapBookingStartTime(startDate)).endDate(bookingTimeConverter.mapBookingEndTime(endDate)).status(BookingStatus.ACCEPTED).roomType(room.getRoomType()).assignedRoom(room).priceTotal(room.getRoomType().getPricePerNight() * ChronoUnit.DAYS.between(startDate, endDate)).firstName("John").lastName("Smith").idCode("50001010017").email("John.Doe@gmail.com").build());
    }
}
